...
const MAX_PROXY_CHAT_BYTES = 32 * 1024;
...
async function handleProxyRequest(request) {
  ...
  let body;
  try {
    body = await readJsonBodyWithLimit(request, MAX_PROXY_CHAT_BYTES);
  } catch (error) {
  ...
  try {
    ...
    return handlePublicChatRequest({ request, context, rawBody: chatBody });
  } catch (error) {
