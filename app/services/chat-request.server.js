...
export async function handlePublicChatRequest({
  ...
  if (request.method === "GET" && url.searchParams.get("history") === "true") {
    ...
    if (!parsed.success) {
      return jsonResponse(
        { error: "Invalid history request", requestId: context.requestId },
        400,
        responseHeaders,
      );
    }
    ...
    try {
      const scopedContext = withConversationContext(
        context,
      ...
      if (error instanceof ConversationNotFoundError) {
        return jsonResponse(
          { error: "Not found", requestId: context.requestId },
          404,
          responseHeaders,
        );
      }
      ...
      return jsonResponse(
        { error: "History is unavailable", requestId: context.requestId },
        500,
        responseHeaders,
      );
    }
  ...
  if (request.method !== "POST") {
    return jsonResponse(
      { error: "Method not allowed", requestId: context.requestId },
      405,
      responseHeaders,
    );
  }
  ...
  if (contentLength > MAX_REQUEST_BYTES) {
    return jsonResponse(
      { error: "Request is too large", requestId: context.requestId },
      413,
      responseHeaders,
    );
  }

  let rawBody = suppliedRawBody;
  if (rawBody === undefined) {
    try {
      ...
      return jsonResponse(
        { error: "Invalid JSON request", requestId: context.requestId },
        400,
        responseHeaders,
      );
    }
  ...
  if (!parsed.success) {
    logger.warn("Chat payload rejected", {
      validation: formatZodError(parsed.error),
    });
    return jsonResponse(
      { error: "Invalid chat request", requestId: context.requestId },
      400,
      responseHeaders,
    );
  }
  if (
    ...
    parsed.data.visitor_id !== context.visitorId
  ) {
    return jsonResponse(
      { error: "Unauthorized", requestId: context.requestId },
      401,
      responseHeaders,
    );
  }
  ...
  const scopedContext = parsed.data.conversation_id
    ...
    : context;
  const visitorId = context.visitorId || parsed.data.visitor_id || undefined;
  let rateLimit;
  try {
    rateLimit = await consumeWidgetRateLimit(scopedContext, {
      visitorId,
    });
  } catch (error) {
    logger.error("Shared rate limiter unavailable", { error });
    return jsonResponse(
      {
  ...
  const responseStream = createSseStream(
    async (stream) => {
      try {
