...
} from "./lib/fetch-with-timeout.server";
import { createLogger } from "./lib/logger.server";
import {
  assertTrustedShopifyUrl,
  normalizeShopDomain,
} from "./security/shopify-domain.server";
...
export class McpRequestError extends Error {
  constructor(code, message, { status = 502 } = {}) {
    ...
    this.status = status;
    this.publicMessage =
      "Shopify could not complete the requested store action.";
  }
}
...
class MCPClient {
  ...
  async initialize() {
    ...
    if (storefront.status === "rejected") {
      this.logger.warn("Storefront MCP discovery failed", {
        error: storefront.reason,
      });
    }
    if (customer.status === "rejected") {
      this.logger.warn("Customer MCP discovery failed", {
        error: customer.reason,
      });
    }
  ...
  async connectToStorefrontServer() {
    ...
    this.logger.info("Storefront MCP connected", {
      toolCount: this.storefrontTools.length,
    });
  ...
  async connectToCustomerServer() {
    const token = this.context.conversationId
      ? await getCustomerToken(this.context, {
    ...
    this.logger.info("Customer MCP connected", {
      toolCount: this.customerTools.length,
    });
  ...
  async callTool(toolName, toolArgs, options = {}) {
    ...
    if (
      isKnownCustomerTool ||
      this.customerTools.some((tool) => tool.name === toolName)
    ) {
      return this.callCustomerTool(toolName, toolArgs, options);
    }
    if (this.storefrontTools.some((tool) => tool.name === toolName)) {
      return this.callStorefrontTool(toolName, toolArgs, options);
    }

    throw new McpRequestError(
      "TOOL_NOT_AVAILABLE",
      `Shopify tool ${toolName} is unavailable`,
      {
        status: 409,
      },
    );
  ...
  async callStorefrontTool(toolName, toolArgs, options = {}) {
    const response = await this.makeJsonRpcRequest(
      this.storefrontMcpEndpoint,
      "tools/call",
  ...
  async callCustomerTool(toolName, toolArgs, options = {}) {
    try {
      const response = await this.makeJsonRpcRequest(
        this.customerMcpEndpoint,
        "tools/call",
  ...
  async makeJsonRpcRequest(
    ...
    if (!response.ok) {
      ...
      throw new McpRequestError(
        "MCP_REJECTED",
        "Shopify MCP request was rejected",
        {
          status: response.status,
        },
      );
    ...
    try {
      ...
      throw new McpRequestError(
        ...
        "Shopify MCP returned an invalid response",
      );
    }
  }
