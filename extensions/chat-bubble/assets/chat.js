...
(function () {
  ...
  const safeSessionStorage = {
    getItem: function (key) {
      try {
        return (
          window.sessionStorage.getItem(key) ?? sessionMemory.get(key) ?? null
        );
      } catch (_error) {
    ...
    setItem: function (key, value) {
      ...
      sessionMemory.set(key, normalized);
      try {
        window.sessionStorage.setItem(key, normalized);
      } catch (_error) {
        // The in-memory value keeps the current page usable in privacy modes.
      }
    },
    removeItem: function (key) {
      ...
      try {
        ...
        // Nothing else is required after removing the in-memory value.
      }
    },
  };
  ...
  const ShopAIChat = {
    ...
    Config: {
      defaults: {
        ...
        widget: {
          ...
          primaryCtaLabel:
            window.shopChatConfig?.widget?.primaryCtaLabel || "Shop with chat",
          launcherLabel:
            window.shopChatConfig?.widget?.launcherLabel || "Ask Sage",
          launcherStyle:
            window.shopChatConfig?.widget?.launcherStyle || "label",
          cornerRadius: window.shopChatConfig?.widget?.cornerRadius || 12,
          colors: window.shopChatConfig?.widget?.colors || {},
          behavior: {
            openOnLoad:
              window.shopChatConfig?.widget?.behavior?.openOnLoad || false,
            showQuickActions:
              window.shopChatConfig?.widget?.behavior?.showQuickActions !==
              false,
            entryBehavior:
              window.shopChatConfig?.widget?.behavior?.entryBehavior || "auto",
            allowFullscreen: true,
            maxProactivePerSession: 1,
            contextualSuggestion: false,
          },
        },
      },
      quickActionPrompts: {
        "Find the right product":
          "I need help finding the right product for me. Ask me what matters and recommend the best options.",
        "Compare options":
          "Compare the best products for my needs and explain which one I should choose.",
        "Ready to buy":
          "Show me products that are available now and help me add the best one to cart.",
      },
      merchantConfig: null,
      ...
      load: async function () {
        ...
        try {
          ...
          console.warn(
            "[IntentCart] merchant config unavailable, using storefront defaults:",
            error.message,
          );
      ...
      getEffectiveConfig: function () {
        ...
        if (themeWidgetConfig) {
          mergedConfig.widget = this.mergeConfig(
            mergedConfig.widget || {},
            themeWidgetConfig,
          );
        }
      ...
      applyToWindowConfig: function (config) {
        const themeWidgetConfig = window.shopChatConfig?.widget || {};
        const mergedWidgetConfig = this.mergeConfig(
          config.widget || this.defaults.widget,
          themeWidgetConfig,
        );
        ...
        window.shopChatConfig = {
          ...
          welcomeMessage:
            ...
            this.defaults.assistant.welcomeMessage,
          quickActions:
            config.assistant?.quickActions ||
            this.defaults.assistant.quickActions,
    ...
    UI: {
      ...
      init: function (container) {
        ...
        this.elements = {
          ...
          continueBrowsingButton: container.querySelector(
            ".shop-ai-continue-browsing",
          ),
          ...
          assistantNameNodes: container.querySelectorAll(
            "[data-shop-ai-assistant-name]",
          ),
        };
      ...
      setupEventListeners: function () {
        ...
        promptButtons.forEach((button) => {
          button.addEventListener("click", () => {
            chatInput.value =
              button.dataset.shopAiPrompt || button.textContent.trim();
            ShopAIChat.Message.send(chatInput, messagesContainer);
          });
        });
      ...
      applyMerchantConfig: function (container) {
        ...
        if (colors.primary)
          container.style.setProperty("--shop-ai-primary", colors.primary);
        ...
        if (colors.text)
          container.style.setProperty("--shop-ai-text", colors.text);
        if (colors.accent)
          container.style.setProperty("--shop-ai-accent", colors.accent);

        if (this.elements.startChatButton && widgetConfig.primaryCtaLabel) {
          this.elements.startChatButton.textContent =
            widgetConfig.primaryCtaLabel;
        }

        if (widgetConfig.cornerRadius !== undefined) {
          const radius = Math.max(
            0,
            Math.min(24, Number(widgetConfig.cornerRadius) || 0),
          );
        ...
        this.elements.assistantNameNodes?.forEach((node) => {
          node.textContent =
            assistantConfig.name || ShopAIChat.Config.defaults.assistant.name;
        });

        this.renderQuickActions(
          assistantConfig.quickActions,
      ...
      applyWidgetPosition: function (container, position) {
        ...
        const safePosition = supportedPositions.includes(position)
          ? position
          : "bottom-right";
      ...
      getWidgetLayout: function () {
        return this.normalizeLayout(
          ShopAIChat.Config.getEffectiveConfig().widget?.layout,
        );
      },
      ...
      applyInitialWidgetBehavior: function () {
        ...
        if (
          behavior.contextualSuggestion &&
          this.canShowContextualSuggestion()
        ) {
          window.setTimeout(() => {
            const { choicePanel, chatWindow } = this.elements;
            const isAlreadyOpen =
              choicePanel?.classList.contains("active") ||
              chatWindow?.classList.contains("active");
            if (document.visibilityState !== "visible" || isAlreadyOpen) return;

            this.incrementContextualSuggestionCount();
            this.openChoicePanel();
            ShopAIChat.API.recordExperimentExposure();
          }, 2000);
          return;
        }
      ...
      canShowContextualSuggestion: function () {
        const maximum = Number(
          this.getWidgetBehavior().maxProactivePerSession || 0,
        );
        if (maximum <= 0) return false;
        try {
          return (
            Number(
              safeSessionStorage.getItem(this.contextualSuggestionKey()) || 0,
            ) < maximum
          );
        } catch (_error) {
          return false;
      ...
      incrementContextualSuggestionCount: function () {
        try {
          const key = this.contextualSuggestionKey();
          const count = Number(safeSessionStorage.getItem(key) || 0);
          safeSessionStorage.setItem(key, String(count + 1));
        } catch (_error) {
          // Storage can be unavailable in strict privacy modes.
        }
      },

      contextualSuggestionKey: function () {
        const experiment = ShopAIChat.Config.getEffectiveConfig().experiment;
        const key = experiment?.key || "launcher_entry_v1";
        return `shopAiContextualSuggestion:${key}`;
      },
      ...
      renderQuickActions: function (quickActions, behavior) {
        ...
        actions.forEach((label) => {
          ...
          button.type = "button";
          button.dataset.shopAiPrompt =
            ShopAIChat.Config.getQuickActionPrompt(label);
          button.textContent = label;
          suggestionsContainer.appendChild(button);
        });

        this.elements.promptButtons = suggestionsContainer.querySelectorAll(
          "[data-shop-ai-prompt]",
        );
      },
      ...
      openChoicePanel: function () {
        const { container, chatBubble, choicePanel, chatWindow } =
          this.elements;
        const layout = this.getWidgetLayout();
    ...
    Message: {
      ...
      send: async function (chatInput, messagesContainer) {
        ...
        const conversationId = safeSessionStorage.getItem(
    ...
    Formatting: {
      ...
      formatMessageContent: function (element) {
        ...
        element.replaceChildren(
          this.buildSafeFragment(element.dataset.rawText),
        );
      },
      ...
      safeCheckoutUrl: function (value) {
        ...
        return path.includes("checkout") || path.startsWith("/cart/c/")
          ? url
          : null;
      },
      ...
      isCurrentShopHost: function (hostname) {
        ...
        const canonical =
          ShopAIChat.API.bootstrapData?.shop?.domain?.toLowerCase();
        let storefrontHost = window.location.hostname.toLowerCase();
        try {
          storefrontHost = new URL(
            ShopAIChat.API.bootstrapData?.shop?.storefrontOrigin ||
      ...
      isTrustedAuthUrl: function (url) {
        ...
        const shopDomain =
          ShopAIChat.API.bootstrapData?.shop?.domain?.toLowerCase() || "";
        const accountHost = shopDomain.replace(
          /\.myshopify\.com$/,
    ...
    API: {
      ...
      ensureBootstrap: async function (force = false) {
        ...
        const appProxyBaseUrl =
          configuredBaseUrl?.startsWith("/") && configuredBaseUrl !== "/"
            ? this.getBackendBaseUrl()
            : "/apps/intentcart";
        const visitorId = this.getVisitorId();
        const response = await fetch(
          `${appProxyBaseUrl}/bootstrap?visitor_id=${encodeURIComponent(visitorId)}`,
          {
            method: "GET",
        ...
        if (!response.ok)
          throw new Error(`Widget bootstrap failed with ${response.status}`);
        const payload = await response.json();
        if (!payload?.token || !payload?.config)
          throw new Error("Widget bootstrap response is invalid");
        if (payload.visitorId && payload.visitorId !== visitorId) {
          safeSessionStorage.setItem("shopAiVisitorId", payload.visitorId);
        }
        this.bootstrapData = payload;
        return payload;
      },

      recordExperimentExposure: async function () {
        const experiment = this.bootstrapData?.config?.experiment;
        if (!experiment?.key || !experiment?.variant) return false;

        const storageKey = `shopAiExperimentExposure:${experiment.key}:${experiment.variant}`;
        try {
          if (safeSessionStorage.getItem(storageKey) === "recorded")
            return false;
        } catch (_error) {
          // The server-side compare-and-set remains the source of truth.
        }
        if (this.exposurePromise) return this.exposurePromise;

        this.exposurePromise = (async () => {
          const endpoint =
            this.bootstrapData?.endpoints?.experimentExposure ||
            "/apps/intentcart/experiment-exposure";
          ...
          try {
            safeSessionStorage.setItem(storageKey, "recorded");
          } catch (_error) {
            // A repeated call is harmless because the server records once.
          }
          return true;
        })();

        try {
          return await this.exposurePromise;
        } catch (_error) {
          return false;
        } finally {
          this.exposurePromise = null;
        }
      },
      ...
      isAppProxyRequestUrl: function (url) {
        try {
          ...
          const isAdvertisedProxyEndpoint = Object.values(
            this.bootstrapData?.endpoints || {},
          ).some((endpoint) => {
            const advertised = new URL(endpoint, window.location.origin);
            return (
              advertised.origin === window.location.origin &&
              advertised.pathname === target.pathname
            );
          });
          return (
            target.origin === window.location.origin &&
            (isAdvertisedProxyEndpoint ||
      ...
      withAppProxyCredential: function (url, options = {}) {
        if (
          !this.isAppProxyRequestUrl(url) ||
        ...
        try {
          ...
          return {
            ...
            body: JSON.stringify({
              ...body,
              widget_token: this.bootstrapData.token,
            }),
          };
        } catch (_error) {
          return options;
      ...
      authorizedFetch: async function (url, options = {}, retry = true) {
        ...
        const requestOptions = this.withAppProxyCredential(url, options);
        const response = await fetch(url, {
          ...requestOptions,
          headers: this.getAuthorizedHeaders(requestOptions.headers || {}),
        });
        if (response.status !== 401 || !retry) return response;

        await this.ensureBootstrap(true);
        const retryOptions = this.withAppProxyCredential(url, options);
        return fetch(url, {
          ...retryOptions,
          headers: this.getAuthorizedHeaders(retryOptions.headers || {}),
        });
      },

      getVisitorId: function () {
        let visitorId = safeSessionStorage.getItem("shopAiVisitorId");
        if (!visitorId) {
          visitorId = createUuid();
          safeSessionStorage.setItem("shopAiVisitorId", visitorId);
        }
        return visitorId;
      },
      ...
      handleStreamEvent: function (
        ...
        switch (data.type) {
          ...
          case "auth_required":
            ...
            safeSessionStorage.setItem("shopAiLastMessage", userMessage || "");
            {
              const authUrl = ShopAIChat.Formatting.safeHttpUrl(
          ...
          case "cart_state":
            {
              const checkoutUrl = ShopAIChat.Formatting.safeCheckoutUrl(
                data.checkoutUrl,
              );
          ...
          case "tool_use":
            if (data.tool_use_message) {
              ShopAIChat.Message.addProgress(
      ...
      fetchChatHistory: async function (conversationId, messagesContainer) {
        try {
          ...
          const proxyHistoryUrl =
            this.bootstrapData?.endpoints?.history ||
            this.buildBackendUrl("/history");
          const usesAppProxy = this.isAppProxyRequestUrl(proxyHistoryUrl);
          const historyUrl = usesAppProxy
            ? proxyHistoryUrl
          ...
          safeSessionStorage.removeItem("shopAiConversationId");
        }
      },
    },
    ...
    Auth: {
      ...
      openAuthPopup: function (authUrlOrElement) {
        ...
        if (
          !safeAuthUrl ||
          !ShopAIChat.Formatting.isTrustedAuthUrl(safeAuthUrl)
        ) {
      ...
      startTokenPolling: function (conversationId, messagesContainer) {
        ...
        safeSessionStorage.setItem("shopAiTokenPollingId", pollingId);
        ...
        const poll = async () => {
          if (
            safeSessionStorage.getItem("shopAiTokenPollingId") !== pollingId
          ) {
            return;
          }
          ...
          try {
            ...
            const proxyTokenUrl =
              ShopAIChat.API.bootstrapData?.endpoints?.tokenStatus ||
              ShopAIChat.API.buildBackendUrl("/auth/token-status");
            const usesAppProxy =
              ShopAIChat.API.isAppProxyRequestUrl(proxyTokenUrl);
            const tokenUrl = usesAppProxy
              ? proxyTokenUrl
              : ShopAIChat.API.buildBackendUrl(
            ...
            if (data.status === "authorized") {
              const message = safeSessionStorage.getItem("shopAiLastMessage");

              if (message) {
                safeSessionStorage.removeItem("shopAiLastMessage");
                setTimeout(() => {
                  ShopAIChat.Message.add(
              ...
              safeSessionStorage.removeItem("shopAiTokenPollingId");
              return;
            }
    ...
    Product: {
      ...
      createCard: function (product) {
        ...
        const safeImageUrl = ShopAIChat.Formatting.safeImageUrl(
          product.image_url,
        );
        ...
        const safeProductUrl = ShopAIChat.Formatting.safeProductUrl(
          product.url,
        );
        if (safeProductUrl) {
          ...
          return product.options
            .map((option) => {
              ...
              const values = Array.isArray(option.values)
                ? option.values
                    .map(ShopAIChat.Product.formatOptionValue)
                    .filter(Boolean)
                    .join(", ")
                : "";
    ...
    init: async function () {
      ...
      const conversationId = safeSessionStorage.getItem("shopAiConversationId");
