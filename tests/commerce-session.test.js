...
import {
  ...
  chunkText,
} from "../app/services/knowledge.server";
...
describe("tenant-scoped commerce persistence", () => {
  beforeEach(() => {
    ...
    database.knowledgeSource.create.mockImplementation(({ data }) =>
  ...
  it("allows only explicit commerce journey transitions", () => {
    ...
    expect(() => assertJourneyTransition("DISCOVER", "CART")).toThrow(
      InvalidJourneyTransitionError,
    );
    expect(() => assertJourneyTransition("COMPLETED", "DISCOVER")).toThrow(
      InvalidJourneyTransitionError,
    );
  });
  ...
  it("always scopes reads, optimistic updates, and messages by shop", async () => {
    ...
    expect(database.commerceSession.findFirst).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: "commerce-1", shopId: "shop-alpha" },
      }),
    );

    database.commerceSession.findFirst.mockResolvedValue({
      id: "commerce-1",
      shopId: "shop-alpha",
      version: 4,
    ...
    expect(database.commerceSession.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("expires a stale session and starts a new conversation", async () => {
    database.transaction.conversation.findFirst.mockResolvedValue({
      id: "conversation-old",
    });
    ...
    database.transaction.commerceSession.update.mockResolvedValue({
      id: "commerce-old",
    });
    database.transaction.conversation.update.mockResolvedValue({
      id: "conversation-old",
    });
  ...
  it("injects the tenant into knowledge writes and retrieval", async () => {
    ...
    expect(retriever.search).toHaveBeenCalledWith(context, {
      query: "returns",
    });
  });
  ...
  it("chunks merchant knowledge deterministically within the configured bound", () => {
    ...
    expect(() => chunkText("content", 100)).toThrow(
      "Invalid knowledge chunk size",
    );
  });
  ...
  it("reads merchant configuration through the current shop only", async () => {
    database.merchantConfig.findUnique.mockResolvedValue({
      ...
      welcomeMessage:
        "Tell me what you are looking for and I will help you choose.",
