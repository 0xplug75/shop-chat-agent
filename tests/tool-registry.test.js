...
function createRegistry() {
  const catalogAdapter = {
    searchCatalog: vi.fn().mockResolvedValue({
      products: [product, { ...product, id: "product-2" }, { ...product, id: "product-3" }, { ...product, id: "product-4" }]
    }),
    getProduct: vi.fn().mockResolvedValue({ product })
  };
  const policyAdapter = {
    searchPolicies: vi.fn().mockResolvedValue({ response: { content: [{ text: "30 day returns" }] } })
  };
  const cartAdapter = {
    getCart: vi.fn().mockResolvedValue({ cartId: "cart-1", cart: { id: "cart-1", lines: [] } }),
    addConfirmedItem: vi.fn().mockResolvedValue({
      cartId: "cart-1",
      cart: { id: "cart-1", checkoutUrl: "https://alpha.myshopify.com/checkouts/cn/one" },
      businessMessage: { outcome: "ok", assistantMessage: "Shopify completed the requested commerce action." }
    })
  };
  const checkoutAdapter = {
    getCheckoutUrlFromCartOrCheckout: vi.fn().mockReturnValue("https://alpha.myshopify.com/checkouts/cn/one"),
  ...
  const knowledgeService = {
    searchKnowledge: vi.fn().mockResolvedValue([{ content: "Approved answer", reference: { title: "FAQ" } }])
  };
  return {
...
describe("typed Shopify tool registry", () => {
  ...
  it("caps catalog recommendations at three", async () => {
    const result = await tools.registry.execute("search_catalog", {
  ...
  it("requires an exact second-turn confirmation before changing a cart", async () => {
    ...
    const second = await tools.registry.execute("update_cart", input, {
      ...
      confirmation: { accepted: true, ...input }
    });
  ...
  it("rejects unknown variants and cross-session cart identifiers", async () => {
    await expect(tools.registry.execute("update_cart", {
      productId: product.id,
    ...
    await expect(tools.registry.execute("get_cart", { cartId: "cart-other" }, {
  ...
  it("uses approved tenant knowledge before the Shopify policy fallback", async () => {
    const result = await tools.registry.execute("retrieve_brand_knowledge", {
      query: "returns",
      limit: 5,
      maxCharacters: 6000
    }, {
